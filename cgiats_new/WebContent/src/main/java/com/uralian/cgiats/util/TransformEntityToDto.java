...
//				dto.getCandidateDto().setStreet2(submittal.getCandidate().getAddress().getStreet2()!=null ? submittal.getCandidate().getAddress().getStreet2() : "");
	...
	public static ClientInfoDto getprojectInfoDetails(ClientInfo clientInfo, AgencyDetails agencyDetails) {
		...
		if (clientInfo != null) {
			...
			clientDto.setBillingPartner(Utils.replaceNullWithEmpty(clientInfo.getBillingPartner()));
			clientDto.setStrProjectEndDate(Utils.replaceNullWithEmpty(clientInfo.getStrProjectEndDate()));
			
			clientDto.setEmployeeId(candidateInfo.getEmployeeId());
			clientDto.setDateOfLastContact(Utils.convertDateToString(candidateInfo.getDateOfLastContact()));
			clientDto.setEmpRole(candidateInfo.getEmpRole());
			clientDto.setRating(candidateInfo.getRating());
			clientDto.setCurrentStatus(candidateInfo.getCurrentStatus());
			clientDto.setGlassDoorReview(candidateInfo.getGlassDoorReview());
			clientDto.setNotes(candidateInfo.getNotes());
			clientDto.setReferralNotes(candidateInfo.getReferralNotes());
			clientDto.setPreferredName(candidateInfo.getPreferredName());
			clientDto.setEmpOtherRole(candidateInfo.getEmpOtherRole());
