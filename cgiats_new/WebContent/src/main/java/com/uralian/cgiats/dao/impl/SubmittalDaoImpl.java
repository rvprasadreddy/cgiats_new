...
public class SubmittalDaoImpl extends GenericDaoImpl<Submittal, Integer> implements SubmittalDao {
	...
	public Map<SubmittalStatus, Integer> getAllSubmittalStatusCounts(Date dateStart, Date dateEnd) {
		try {
			...
			if(userMap.get(SubmittalStatus.INTERVIEWING_2)!=null){
				userMap.put(SubmittalStatus.INTERVIEWING, userMap.get(SubmittalStatus.INTERVIEWING_2)+(userMap.get(SubmittalStatus.INTERVIEWING)!=null?userMap.get(SubmittalStatus.INTERVIEWING):0));
			}
/*
			for (Object record : result) {
				Object[] tuple = (Object[]) record;
	...
	public List<CandidateDto> getOutofProjs(CandidateSearchDto statusDto) {
		try {
			...
			hql.append("select distinct c.id, c.firstName, c.lastName, c.address, c.email, c.phone, c.visaType, c.updatedBy, c.updatedOn,c.title, c.hot, c.block from Candidate c "
					+ "where c.id in (select sub.candidate.id from Submittal sub where sub.id in (select s.id from Submittal s ,SubmittalEvent h ,User u where s.id=h.submittal.id "
					+ "and h.status in ('STARTED') and s.deleteFlag=0 and s.status in ('OUTOFPROJ') and s.createdBy=u.userId and (u.userId in "
					+ "( select ua.userId from User ua where ua.userRole in ('DM','ADM') and ua.status='ACTIVE') "
					+ "or u.assignedBdm in "
					+ "(select ua.userId from User ua where ua.userRole in ('DM','ADM') and ua.status='ACTIVE') )");
			
			 
			if (fromDate != null) {
				hql.append("  and DATE (COALESCE(h.updatedOn, h.createdDate)) >= :fromDate");
				hql.append("  and DATE (COALESCE(s.createdDate, s.createdOn)) >= :fromDate");
				paramNames.add("fromDate");
				paramValues.add(fromDate);
			}
			
			if (toDate != null) {
				hql.append("  and DATE (COALESCE(h.updatedOn, h.createdDate)) <=:toDate ");
				hql.append("  and DATE (COALESCE(s.createdDate, s.createdOn)) <=:toDate ");
				paramNames.add("toDate");
				paramValues.add(toDate);
			}
			
			hql.append("  group by s.id)) ORDER By c.updatedOn DESC");
			
			/* select * from candidate c where c.candidate_id  in 
			 ( select sub.candidate_id from submittal sub where sub.submittal_id in (
			 select s.submittal_id from submittal s, submittal_history h , user_acct u where s.submittal_id = 
			 h.submittal_id AND (COALESCE(h.updated_on, h.created_on)) >= '09-06-2016' 
			  AND  (COALESCE(h.updated_on, h.created_on)) <= '06-21-2017'
			 AND (COALESCE(s.updated_on, s.created_on)) >= '09-06-2016'
			 AND  (COALESCE(s.updated_on, s.created_on)) <= '06-21-2017' AND h.status 
			  in ('STARTED') AND s.delete_flag = 0 AND s.status in ('OUTOFPROJ')  
			  and s.created_by=u.user_id and  (u.user_id in
			 (select ua.user_id from user_acct ua where ua.user_role in ('DM','ADM') and ua.status='ACTIVE')
			 or u.assigned_bdm in
			 (select ua.user_id from user_acct ua where ua.user_role in ('DM','ADM') and ua.status='ACTIVE') )group by s.submittal_id))
			  and c.delete_flg = 0;*/
	...
	@Override
	public Object getResumeAuditLogDataRecordsCount(String sqlCount) {
