...
//		@Scheduled(cron = "0 24 18 * * ?")
		public void sendMailForRecruiterSubmittalsDailyWise() throws ServiceException {
		 try {
				...
				fromDate.set(cal.get(Calendar.YEAR), 0, 1,0,0,0);
				Date yearStartDate = fromDate.getTime();
				
				
				sendRecruitersSubmittalWisePerformance(toDate, toDate, yearStartDate);
			} catch (Exception e) {
				e.printStackTrace();
				log.error(e.getMessage(), e);
			}
...
//	@Scheduled(cron = "0 24 18 * * ?")
	public void sendMailForRecruiterSubmittalsMonthlyWise() throws ServiceException {
	 try {
			Calendar cal=Calendar.getInstance();
			cal.add(Calendar.DATE, -1);
			Date toDate= cal.getTime();
			...
			if(cal.getActualMaximum(Calendar.DAY_OF_MONTH)==cal.get(Calendar.DATE)){
				...
				monthStartCal.set(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH), 1,0,0,0);
				Date fromDate = monthStartCal.getTime();
				
				Calendar yearStartCal=Calendar.getInstance();
				yearStartCal.set(cal.get(Calendar.YEAR), 0, 1,0,0,0);
				Date yearStartDate = yearStartCal.getTime();
				sendRecruitersSubmittalWisePerformance(fromDate, toDate, yearStartDate);
				
				List<SubmittalDto> list = submittalService.getConsultantInfoReport(Utils.convertStringToDate(Utils.convertDateToString(fromDate)),Utils.convertStringToDate(Utils.convertDateToString(toDate)), null);
				new ExcelGeneratorJobOrderScheduler().createExcelFor_ConsultantInfoReport(list, fromDate, toDate, emailIds(ReportNameConstants.ConsultantInfoReport.toString()));
			}
	 ...
	 private void sendRecruitersSubmittalWisePerformance(Date fromDate,Date toDate,Date yearStartDate){
		 try {
			 log.info("<<<<<<<<<< sendMailForRecruiterSubmittals fromdate:"+fromDate+" todate:"+toDate+" >>>>>>>");
			 Map<String, List<SubmittalDto>> dmWithRecMap = submittalService.getDayWisePerformanceOfRecruiters(Utils.convertStringToDate(Utils.convertDateToString(fromDate)),Utils.convertStringToDate(Utils.convertDateToString(toDate)));
				List<Map<String,Object>> jobOrdersList = jobOrderService.getCreatedJobOrdersOfToday(Utils.convertStringToDate(Utils.convertDateToString(fromDate)),Utils.convertStringToDate(Utils.convertDateToString(toDate)));
				Object object = submittalService.getdmsDetailsReport(yearStartDate, toDate, Utils.getListWithSingleQuote(dmWithRecMap.keySet()), null);
				Map<String,SubmittalDto> recWithPerformanceMap =null;
				if(object!=null){
					Object dtoList =  ((Map<String,Object>)object).get(Constants.GRIDDATA);
					if(dtoList!=null){
						List<SubmittalDto> list = (List<SubmittalDto>)dtoList;
						recWithPerformanceMap = new HashMap<String,SubmittalDto>();
						for(SubmittalDto dto:list){
							recWithPerformanceMap.put(dto.getUserId(), dto);
						}
					}
				...
				if (dmWithRecMap != null && dmWithRecMap.size() > 0) {
					...
					new ExcelGeneratorJobOrderScheduler().createExcelFor_RecDayWisePerformaceWithJobOrders(recWithPerformanceMap,jobOrdersList, dmWithRecMap,fromDate,toDate,emailIds(ReportNameConstants.JobOrdersCategorywiseReport.toString()));
				}
		} catch (Exception e) {
			e.printStackTrace();
			log.error(e.getMessage(), e);
