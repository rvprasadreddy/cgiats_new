	...
	public DocumentHolder call() throws Exception {
		...
		try {
			...
			if (doc != null && doc.getData() != null) {
				...
				if(candidate.getEmail()!=null && candidate.getEmail().trim().length()>0){
					List<Candidate> existEmail = getService().getCandidateDetails(candidate.getEmail());
					if (existEmail.size() > 0) {
						System.out.println("call() 2nd if 3");
						log.debug("Career Builder Email Exists: " + candidate.getEmail());
						log.error("Techfetch Candidate already exists");
						return null;
					} 
				}
