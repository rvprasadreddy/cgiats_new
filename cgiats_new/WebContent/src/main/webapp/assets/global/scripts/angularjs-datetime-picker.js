(function() {
  ...
  var tmpl = [
    ...
    '        selectable: (mv.year < mv.currentYear)||(mv.year <= mv.currentYear && mv.month <= mv.currentMonth && (day <= mv.currentDay ? mv.month <= mv.currentMonth : mv.month < mv.currentMonth)),',
    ...
    '    <label class="hourLabel" ng-show="(mv.year < mv.currentYear)||(mv.year <= mv.currentYear && mv.month <= mv.currentMonth)">Hour:</label> <input class="hourInput" type="range" min="0" max="23" ng-model="inputHour" ng-change="updateNgModel()"  ng-show="(mv.year < mv.currentYear)||(mv.year <= mv.currentYear && mv.month <= mv.currentMonth)"/>',
    '    <label class="minutesLabel" ng-show="(mv.year < mv.currentYear)||(mv.year <= mv.currentYear && mv.month <= mv.currentMonth)">Min:</label> <input class="minutesInput" type="range" min="0" max="59" ng-model="inputMinute"  ng-change="updateNgModel()"  ng-show="(mv.year < mv.currentYear)||(mv.year <= mv.currentYear && mv.month <= mv.currentMonth)"/> ',
    '    <button type="button" class="btn btn-info" style="width:60px;margin-bottom:10px" ng-click="closeDtPicker()" ng-show="(mv.year < mv.currentYear)||(mv.year <= mv.currentYear && mv.month <= mv.currentMonth)">Ok</button><br>',
    '  </div> ',
    '</div>'].join("\n");
        ...
        if (attrs.ngModel) { // need to parse date string
         ...
         //dd-MM-yyyy HH:mm
          if (dateStr) {
            ...
            if(date_Format == 'dd-MM-yyyy HH:mm' || date_Format == 'dd-MM-yyyy HH:mm:ss'){
            	var pattern = /(\d{2})\-(\d{2})\-(\d{4}) (\d{2})\:(\d{2})/;
        		var dateArray = pattern.exec(dateStr); 
        		if(dateArray && dateArray.length > 2){
        		 scope.selectedDate = new Date(parseInt(dateArray[3]), (parseInt(dateArray[2])-1),parseInt(dateArray[1]),parseInt(dateArray[4]),parseInt(dateArray[5]),0);
        		}
//            dateStr = dateStr.replace(/([0-9]{2}-[0-9]{2})-([0-9]{4})/,'$1-$2');      //dd-mm-yyyy to yyyy-mm-dd
            }else{
      ...
      scope.closeDtPicker = function(day) {
    	  var modelDateValue = ''+ctrl.triggerEl.scope().$eval(attrs.ngModel);
    	  if(modelDateValue == 'undefined'){
    		  if(date_Format == 'dd-MM-yyyy HH:mm' || date_Format == 'dd-MM-yyyy HH:mm:ss'){
    			var d = new Date,
      		    dformat = [(d.getDate())<10?("0"+d.getDate()):(d.getDate()),
      		               (d.getMonth()+1)<10?("0"+(d.getMonth()+1)):(d.getMonth()+1),d.getFullYear()].join('-')+' '+
      		              ["09",
      		               "00"].join(':');
    			  var elScope = ctrl.triggerEl.scope(), dateValue;
        		  elScope.$eval(attrs.ngModel + '= date', {date: dformat});
    		  }else{
    		  var d = new Date,
    		    dformat = [(d.getMonth()+1)<10?("0"+(d.getMonth()+1)):(d.getMonth()+1),
    		               (d.getDate())<10?("0"+d.getDate()):(d.getDate()),d.getFullYear()].join('-')+' '+
    		              ["09",
    		               "00"].join(':');
    		   var elScope = ctrl.triggerEl.scope(), dateValue;
     		  elScope.$eval(attrs.ngModel + '= date', {date: dformat});
