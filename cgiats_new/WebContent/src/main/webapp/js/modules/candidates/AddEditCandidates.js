...
(function(angular) {
	...
	.controller('CandidatesController',function($rootScope, $scope, $sessionStorage, $interval,resumesViewAndDownload,
							$http,$state, $stateParams, $mdDialog, $mdMedia,$location,$timeout,$window,dateRangeService,$log) {
			...
			delete $scope.$storage.cgiFileContent;
					...
					if($rootScope.rsLoginUser && $rootScope.rsLoginUser.userId){
						$scope.candidate.atsUserId = $rootScope.rsLoginUser.userId;
							...
							if (candidateId != null) {
								$scope.isResumeContentLoad =false;
								$(".underlay").show();
...
//													alert("block list  "+!$scope.addToBlockListSrc);
						...
						$scope.getResumeContentForEdit = function(){
							if(!$scope.isResumeContentLoad){
								$(".underlay").show();
								$scope.isResumeContentLoad = true;
							var response = $http.get('searchResume/getResumeContentForEdit?candidateId='+$scope.candidateData.candidateId);
							response.success(function(data, status,headers, config) {
								$(".underlay").hide();
							$scope.$storage.originalFileContent = data.resumeContent;
							$scope.$storage.rtrFileContent = data.rtrContent;
