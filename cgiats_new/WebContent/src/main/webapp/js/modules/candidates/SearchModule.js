						...
						function DialogController($scope,$mdDialog,$timeout) {
					    	  ...
					    	  $scope.deleteQuery = function() {
					    		  ...
					    		  var response = $http.get('searchResume/deleteSavedQuery?queryId='+queryId);
					   	          response.success(function(data, status, headers, config) {
//					   	        	location.reload();
					   	        	angular.element("#searchControllerId").scope().reset();
					   	        	$timeout(function() {
						...
						$scope.SavedQueries = function() {
							...
							response
									.success(function(data, status, headers,
										...
										$scope.onlyOnAssignment = qurydata[0].onlyOnAssignment;
										/*alert($scope.onlyOnAssignment);
										if($scope.onlyOnAssignment){
											alert('');
											$('#onlyOnAssignmentId').attr('checked',true);
										}*/
...
//							alert(isCancel);
							...
							if ($scope.advancedSearch = true) {
								...
								$scope.$storage.onlyOnAssignment = $scope.onlyOnAssignment;
								...
								$scope.searchFields = {
									...
									"email" : $scope.email,
									"onlyOnAssignment" : $scope.onlyOnAssignment,
										...
										window.candidateResumeEvents = {
											/*'click .info' : function(e, value,
												...
												'click .view' : function(e,value, row, index) {
													...
													if(!$scope.keyskills && $scope.$storage.keyskills){
														$scope.keyskills = $scope.$storage.keyskills;
													}if(!$scope.keywords && $scope.$storage.keywords){
														$scope.keywords = $scope.$storage.keywords;
													}
													
													$window.open("views/candidates/candidate_popup.html?"+row.id+'?'+$scope.keyskills+'?'+$scope.keywords, "_blank", 'width=900,height=500, left=150, top=50')
													/*var $popup = $window.open("views/candidates/candidate_popup.html?", "popup", 'width=900,height=500, left=150, top=50')
													$popup.candId = row.id;
													$popup.keyskills = $scope.keyskills;
													$popup.keywords = $scope.keywords;*/
...
//											var checkedArray = angular.copy($scope.smsCheck);
						...
						$scope.checkValue = function(value){
							  if(value){
							    $scope.onlyOnAssignment = true;
							  }
							  else 
								  $scope.onlyOnAssignment = false;
							}
