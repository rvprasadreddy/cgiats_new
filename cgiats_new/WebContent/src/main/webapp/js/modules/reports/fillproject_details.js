			   ...
			   response.success(function (data,status,headers,config){
				   ...
				   if(data){
					   for(var i=0;i<data.length;i++){
						   var clientObj={value:data[i],description:data[i]};
						   $scope.clientList.push(clientObj);
					   }
				   }
...
//			$scope.getclientnames();
		...
		$scope.onloadFun = function(){
			...
			$scope.lastContactDateopen = function() {
				$scope.lastContactDatePopup.opened = true;
			};
			$scope.lastContactDatePopup = {
				opened : false
			};
			
			$scope.projectEndDateopen = function() {
				$scope.projectEndDatePopup.opened = true;
			};
			$scope.projectEndDatePopup = {
				opened : false
			};
...
//					alert("success data  "+JSON.stringify(data));
					...
					if(data && data.projectEndDate){
						data.projectEndDate = dateRangeService.convertStringToDate(data.projectEndDate);
					}
					if(data && data.dateOfLastContact){
						data.dateOfLastContact = dateRangeService.convertStringToDate(data.dateOfLastContact);
					}
					
					
					$scope.project = data;
					$scope.project.candidateId = $stateParams.id;
					$scope.project.candidateInfoId = $stateParams.infoId;
					if($scope.project.endDate && $scope.project.endDate!=null){
						$scope.isEndDateKnow = true;
					}else{
						$scope.isEndDateKnow = false;
					}
...
//			alert(JSON.stringify(project));
			...
			if(project.projectEndDate){
				project.projectEndDate = dateRangeService.formatDate(project.projectEndDate);
				}
			if(project.dateOfLastContact){
				project.dateOfLastContact = dateRangeService.formatDate(project.dateOfLastContact);
				}
			
			if($scope.isEndDateKnow){
				project.strProjectEndDate = null;
			}else{
