;(function(angular){
	...
	.controller("createsubmitalscontroller",function( $rootScope, $scope, blockUI, $http, $timeout, $location,
		...
		$scope.candidate = [];
		var candidateData = [];
		var isFromOnLoadForSearch = true;
		...
		$scope.onload = function()
		{
			...
			if($location.search().jobOrder != undefined){
				...
				response.success(function(data, status, headers,
					...
					$('#keyskillRdId').val(data.keySkills);
...
//					$scope.searchFields.title = data.title;
		...
		$scope.searchbyjoborderdetails = function(){
			...
			if($scope.searchBy.indexOf('Title') != -1){
				$scope.searchFields.title = $scope.title;
			}
			if($scope.searchBy.indexOf('Key Skills') != -1){
				isFromOnLoadForSearch = true;
				$scope.searchFields.keySkill =$scope.keyskills;
			}
			if($scope.searchBy.indexOf('Location') != -1){
...
//										}
//										alert(isFromOnLoadForSearch);
										if(isFromOnLoadForSearch){
//											alert($('#keyskillRdId').val());
											$scope.searchFields.keySkill = $('#keyskillRdId').val();
//											isFromOnLoadForSearch = false;
										}
//										alert($scope.searchFields.keySkill);
